...
void test_MotorController_setupMovement_given_coordinate_from_1_2_3_to_4_6_8_expect_storing_all_info_into_motor_info(void)
{
  MotorInfo Motorinfo[] = {
    ...
    //test all the motor info
    //name; delta; deltaRef; error; Dostepping; isReferencing; start; end,Motorinfo,index
...
/*
motor X is move (1 to 6) delta = 5 ,err = 0 because is being refer                          motor X stepping
motor Y is move (1 to 3) delta = 2 ,err= -1                           ------------------>
motor Z is move (1 to 2) delta = 1 ,err= -3

this testing is check if the motor is being reference then the stepping will be 1(the motor is stepping)
*/
void test_MotorMovement_either_stepping_or_not_given_motorX_is_longest_is_being_referencing_expect_motorXisStepping(void)
{
  MotorInfo Motorinfo[] = {
   //name; delta; deltaRef; error; Dostepping; isReferencing; start; end;
    {'X',5,5,6,0,1,1,6},
...
motor Y is move (1 to 4) delta = 3 ,err =0      ------> motor Z and X is stepping
